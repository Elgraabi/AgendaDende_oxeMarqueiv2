...
public class ClinicaDAOJDBC implements ClinicaDAO {
    ...
    public void update(String nameOfClinic, String address, String phoneNumber, String email, String additionalInformation, Integer idClinic) {
        PreparedStatement pstm = null;

        try {
            conn = FabricaDeConexao.getConnection();
            pstm = conn.prepareStatement(
              "UPDATE CLINIC SET nameOfClinic = ?, address = ?, phoneNumber = ?, email = ?, additionalInformation = ? WHERE idClinic = ?"
            );
            pstm.setString(1, nameOfClinic);
            pstm.setString(2, address);
            pstm.setString(3, phoneNumber);
            pstm.setString(4, email);
            pstm.setString(5, additionalInformation);
            pstm.setInt(6, idClinic);
            pstm.executeUpdate();
        }catch (SQLException e) {
            e.printStackTrace();
        }finally {
